...
class MultiEnvReplayBuffer(object):
    ...
    def __len__(self):
        return (
            self.env_id_to_replay_buffer_map[0].idx
            if not self.env_id_to_replay_buffer_map[0].full
            else capacity
        )
...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        idxs = np.random.choice(self.valid_inds, size=batch_size)

        obses = torch.as_tensor(
            [self.obses[idxs + i] for i in range(self.seq_len)], device=self.device
        ).float()
        actions = torch.as_tensor(
            [self.actions[idxs + i] for i in range(self.seq_len)], device=self.device
        )
        rewards = torch.as_tensor(
            [self.rewards[idxs + i] for i in range(self.seq_len)], device=self.device
        )
        next_obses = torch.as_tensor(
            [self.next_obses[idxs + i] for i in range(self.seq_len)], device=self.device
        ).float()
        not_dones = torch.as_tensor(
            [self.not_dones[idxs + i] for i in range(self.seq_len)], device=self.device
        )
        not_dones_no_max = torch.as_tensor(
            [self.not_dones_no_max[idxs] for i in range(self.seq_len)],
            device=self.device,
        )
