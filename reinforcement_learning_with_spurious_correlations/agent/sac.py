...
class RrexAgent(Agent):
    ...
    def __init__(
        ...
        self.critic_optimizer = torch.optim.Adam(
            list(self.critic.parameters()), lr=critic_lr, betas=critic_betas
        )
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, logger, step):
        ...
        current_Q1, current_Q2, h = self.critic(
            obs, action, return_latent=True, detach=False
        )
    ...
    def update(self, replay_buffer, logger, step, train_sac=True, train_encoder=True):
        ...
        for env_id in range(self.num_envs):
            (
                ...
                not_done,
                not_done_no_max,
            ) = replay_buffer.sample(batch_size, env_id)
            ...
            if train_sac:
                ...
                if step % self.actor_update_frequency == 0:
                    actor_loss, alpha_loss = self.update_actor_and_alpha(
                        obs.view(-1, *(obs.shape[2:])), logger, step
                    )
            ...
            if train_encoder:
                decoder_loss.append(
                    self.update_decoder(
                        obs, action, reward, next_obs, logger, step, env_id
                    )
                )
            if self.penalty_type == "irm":
        ...
        if train_encoder:
            ...
            if self.penalty_type == "rex":
                train_penalty = torch.abs(
                    decoder_loss[0] - decoder_loss[1]
                )  # hardcoded for 2 envs
