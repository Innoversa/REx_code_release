...
class Workspace(object):
    def __init__(self, cfg):
        ...
        cfg.agent.params.obs_dim = (
            self.train_envs[0].observation_space.shape[0] + cfg.noise_dims
        )
    ...
    def run(self):
        ...
        obs, next_obs = [self.train_envs[0].reset()] * self.cfg.num_train_envs, [
            self.train_envs[0].reset()
        ...
        while self.step[0] < self.cfg.num_train_steps:
            for e_idx, env in enumerate(self.train_envs):
                ...
                if (
                    self.step[e_idx] > self.cfg.num_seed_steps
                    and self.step[e_idx] <= self.cfg.num_train_encoder_steps
                ):
                    self.agent.update(
                        self.replay_buffer,
                        self.logger,
