...
)  # if != 0, make 0s and 5s harder than other digit types
parser.add_argument(
    "--digit_shift", type=int, default=0
)  # if != 0, shift probability of hard digits (0s/5s) across domains
...
def make_environment(images, labels, e, grayscale_dup=False, hetero=False, p_hard=0.2):
    ...
    if hetero:
        labels = torch_xor(
            labels,
            torch_bernoulli(0.4, len(labels))
...
for restart in range(config.n_restarts):
    ...
    for step in range(config.steps):
        ...
        print(
            "train acc: %.4f\t valid acc: %.4f" % (log["train_acc"], log["valid_acc"])
        )
