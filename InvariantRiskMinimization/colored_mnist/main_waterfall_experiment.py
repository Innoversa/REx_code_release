...
for restart in range(flags.n_restarts):
    for run in range(flags.runs):
        ...
        envs = [
            make_environment(
                mnist_train[0][::2], mnist_train[1][::2], flags.train_env_1__color_noise
            ),
            make_environment(
                mnist_train[0][1::2],
                mnist_train[1][1::2],
        ...
        pretty_print(
            "step", "train nll", "train acc", "test acc", "irmv1 penalty", "rex penalty"
        )
        ...
        for step in range(flags.steps):
            ...
            penalty_weight = (
                flags.penalty_weight if step >= penalty_anneal_iters_this_run else 1.0
            )
            ...
            if flags.plot or flags.save_numpy_log:
                ...
                all_irmv1_penalties[restart, step] = (
                    irmv1_penalty.detach().cpu().numpy()
                )
