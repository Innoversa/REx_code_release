...
)  # from Section 5.3 of https://arxiv.org/abs/2004.10390

parser.add_argument("--test_prob_class_0", type=float, default=0.5)
...
for restart in range(flags.n_restarts):
    ...
    def make_environment(images, labels, e, p, grayscale_dup=False):
        ...
        all_colors0 = torch.scatter(
            torch.zeros_like(colors), 0, colors_0s.squeeze(1), colors_branch0
        )
        all_colors1 = torch.scatter(
            torch.zeros_like(colors), 0, colors_0s.squeeze(1), 1 - colors_branch0
        )

        all_colors2 = torch.scatter(
            torch.zeros_like(colors), 0, colors_1s.squeeze(1), colors_branch1
        )
        all_colors3 = torch.scatter(
            torch.zeros_like(colors), 0, colors_1s.squeeze(1), 1 - colors_branch1
        )
    ...
    envs = [
        make_environment(
            mnist_train[0][::2], mnist_train[1][::2], flags.train_env_1__color_noise, p1
        ),
        make_environment(
            mnist_train[0][1::2],
            mnist_train[1][1::2],
            flags.train_env_2__color_noise,
            p2,
        ),
        make_environment(
            mnist_val[0],
            mnist_val[1],
            flags.test_env__color_noise,
            flags.test_prob_class_0,
        ),
        make_environment(
            mnist_val[0],
            mnist_val[1],
    ...
    for step in range(flags.steps):
        ...
        min_both = min(
            train_acc.detach().cpu().numpy(), test_acc.detach().cpu().numpy()
        )
        ...
        if flags.plot or flags.save_numpy_log:
            ...
            all_grayscale_test_accs[restart, step] = (
        ...
        log_dict.update(
            {
                "min_of_train_acc_and_test_acc": min(
                    train_acc.detach().cpu().numpy(), test_acc.detach().cpu().numpy()
                ).item(),
    ...
    if flags.plot_fig5_irm_paper:
        ...
        plt.plot(
            [float(_) for _ in h_d[0].keys()],
            [sum(_) / len(_) for _ in h_d[0].values()],
            color="tab:blue",
            marker=".",
            linestyle="",
        )
        plt.plot(
            [float(_) for _ in h_d[1].keys()],
            [sum(_) / len(_) for _ in h_d[1].values()],
            color="tab:orange",
            marker=".",
            linestyle="",
        )
        plt.plot(
            [float(_) for _ in h_d[2].keys()],
            [sum(_) / len(_) for _ in h_d[2].values()],
            color="tab:green",
            marker=".",
            linestyle="",
        )
    ...
    min_of_final_train_acc_and_final_test_accs.append(
        min(train_acc.detach().cpu().numpy(), test_acc.detach().cpu().numpy())
    )
...
log_dict.update(
    {
        "mean__max__min_of_train_acc_and_test_acc": np.mean(
            max__min_of_train_acc_and_test_accs
        ).item(),
        "std__max__min_of_train_acc_and_test_acc": np.std(
            max__min_of_train_acc_and_test_accs
        ).item(),
        "mean__final__min_of_train_acc_and_test_acc": np.mean(
            min_of_final_train_acc_and_final_test_accs
        ).item(),
        "std__final__min_of_train_acc_and_test_acc": np.std(
            min_of_final_train_acc_and_final_test_accs
        ).item(),
