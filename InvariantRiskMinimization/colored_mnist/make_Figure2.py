...
parser.add_argument("--birm_v2_amount", type=float, default=1.0)
parser.add_argument("--birm_v2_schedule", type=str, default="IRM")
parser.add_argument("--waterfall", type=int, default=100)
parser.add_argument(
    "--birm_v2b", type=int, default=0
)  # if 1, use entropy instead of MSE
...
parser.add_argument(
    "--n_val_sets", type=int, default=10
)  # (actually this number + 1)Aa
parser.add_argument(
    "--waterfall_value", type=int, default=10**4
)  # (actually this number + 1)
...
if flags.n_val_sets > 0:
    ...
    val_noise_levels = 1.0 / n_val_sets * np.arange(n_val_sets + 1)
else:
    n_val_sets = 2
    val_noise_levels = [0.15, 0.9]
...
for birm_v2_amount, waterfall_value in zip([0, 1, 1], [0, 100, 10000]):
    ...
    for restart in range(flags.n_restarts):
        ...
        loss_weighting = 0.5 * torch.ones([2])
        if use_cuda:
        ...
        for n, noise in enumerate(val_noise_levels):
            envs.append(
                make_environment(
                    mnist_val[0][n::n_val_sets], mnist_val[1][n::n_val_sets], noise
                )
            )
        ...
        if flags.opt == "Adam":
            optimizer = optim.Adam(
                mlp.parameters(), lr=flags.lr, betas=[flags.momentum, 0.999]
            )
        else:
            optimizer = optim.SGD(
                mlp.parameters(), lr=flags.lr, momentum=flags.momentum
            )
        ...
        for step in range(flags.steps):
            ...
            loss += (
                flags.birm_v1_amount
                * (
                    (loss_weighting[0] * loss1 * noise[0]).mean()
                    + (loss_weighting[1] * loss2 * noise[1])
                ).mean()
            )
            if flags.birm_v2_schedule == "IRM":
                ...
                if step > flags.waterfall:
                    if flags.birm_v2b:
                        loss += (
                            birm_v2_amount
                            * waterfall_value
                            * (compute_softmax_negentropy(loss1, loss2))
                        )
                    else:
                        loss += (
                            birm_v2_amount
                            * waterfall_value
                            * ((loss1 * noise[0]).mean() - (loss2 * noise[1]).mean())
                            ** 2
                        )
                else:
                    if flags.birm_v2b:
                ...
                if flags.birm_v2b:
                    loss += (
                        birm_v2_amount
                        * float(flags.birm_v2_schedule) ** step
                        * (compute_softmax_negentropy(loss1, loss2))
                    )
                else:
                    loss += (
                        birm_v2_amount
                        * float(flags.birm_v2_schedule) ** step
                        * ((loss1 * noise[0]).mean() - (loss2 * noise[1]).mean()) ** 2
                    )
            ...
            if step % flags.eval_interval == 0:
                ...
                all_train_penalties[restart, step] = (
    ...
    color_lookup = {
        0.0: plt.get_cmap("tab10").colors[0],
        100.0: plt.get_cmap("tab10").colors[1],
        10000.0: plt.get_cmap("tab10").colors[2],
    }

    if flags.use_linestyles:
        linestyles = {0.0: "-", 100.0: "--", 10000.0: "dotted"}
    else:
        linestyles = {0.0: "-", 100.0: "-", 10000.0: "-"}
    ...
    plot(
        val_noise_levels,
        [
            envs[2 + n]["acc"].detach().cpu().numpy()
            for n in range(len(val_noise_levels))
        ],
        label=r"$\beta=$" + str(waterfall_value),
        color=color_lookup[waterfall_value],
        linestyle=linestyles[waterfall_value],
    )
    # TODO: hardcoded
    plot(
        0.1,
    ...
    plot(
        0.1,
        envs[3]["nll"].detach().cpu().numpy(),
        "o",
        color=color_lookup[waterfall_value],
    )
    plot(
        0.2,
        envs[4]["nll"].detach().cpu().numpy(),
        "o",
        color=color_lookup[waterfall_value],
    )
    xlabel(r"$P(Y=0|\mathrm{color=red})$")
    ...
    if 0:  # plot shape for side_by_side
        subplots_adjust(
            top=0.97, bottom=0.145, left=0.06, right=0.99, hspace=0.2, wspace=0.12
        )
