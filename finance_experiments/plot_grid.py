...
if __name__ == "__main__":
    ...
    if sys.argv[-1] == "all":
        # combine
        mat_erm, mat_irm, mat_rex = [
            np.loadtxt("results/%s_matrix.dat" % k) for k in ["erm", "irm", "rex"]
        ]
        ...
        print(
            "irm rel performance (mean, std, min, max)",
            ir.mean(),
            ir.std(),
            ir.min(),
            ir.max(),
        ...
        print(
            "rex rel performance (mean, std, min, max)",
            rr.mean(),
            rr.std(),
            rr.min(),
            rr.max(),
        ...
        print(
            "erm performance (mean, std, min, max)", e.mean(), e.std(), e.min(), e.max()
        )
        print(
            "irm performance (mean, std, min, max)", i.mean(), i.std(), i.min(), i.max()
        )
        print(
            "rex performance (mean, std, min, max)", r.mean(), r.std(), r.min(), r.max()
        )
    ...
    for i, (e_tr, m_tr, e_te, m_te) in enumerate(
        zip(train_envs, train_means, test_envs, test_means)
    ):
