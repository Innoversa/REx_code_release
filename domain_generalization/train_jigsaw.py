...
def get_args():
    ...
    parser.add_argument(
        "--learning_rate", "-l", type=float, default=0.01, help="Learning rate"
    )
    parser.add_argument("--epochs", "-e", type=int, default=30, help="Number of epochs")
    parser.add_argument(
        "--n_classes", "-c", type=int, default=31, help="Number of classes"
    )
    parser.add_argument(
        "--jigsaw_n_classes",
        "-jc",
    ...
    parser.add_argument(
        "--jig_weight", type=float, default=0.1, help="Weight for the jigsaw puzzle"
    )
    parser.add_argument(
        "--rex_weight_class",
        type=float,
        default=0.1,
        help="Weight for rex on classsification risk",
    )
    parser.add_argument(
        "--irm_weight_class",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Weight for rex on jigsaw risk",
    )
    parser.add_argument(
        "--irm_weight_jigsaw",
        type=float,
    ...
    parser.add_argument(
        "--ooo_weight", type=float, default=0, help="Weight for odd one out task"
    )
    parser.add_argument(
        "--tf_logger",
        type=bool,
    ...
    parser.add_argument(
        "--TTA", action="store_true", help="Activate test time data augmentation"
    )
    parser.add_argument(
        "--classify_only_sane",
        action="store_true",
...
class Trainer:
    ...
    def _do_epoch(self):
        ...
        for it, ((data, jig_l, class_l), d_idx) in enumerate(self.source_loader):
            ...
            j1_irm = compute_irm_penalty(
                jigsaw_logit[d_idx == 0], jig_l[d_idx == 0], criterion
            )
            j2 = criterion(jigsaw_logit[d_idx == 1], jig_l[d_idx == 1])
            j2_irm = compute_irm_penalty(
                jigsaw_logit[d_idx == 1], jig_l[d_idx == 1], criterion
            )
            j3 = criterion(jigsaw_logit[d_idx == 2], jig_l[d_idx == 2])
            j3_irm = compute_irm_penalty(
                jigsaw_logit[d_idx == 2], jig_l[d_idx == 2], criterion
            )
            ...
            if self.only_non_scrambled:
                if self.target_id is not None:
                    ...
                    rex_class = torch.Tensor([0.0]).cuda()
                    irm_class = torch.Tensor([0.0]).cuda()
                else:
                    class_loss_1 = criterion(
                        class_logit[(jig_l == 0) & (d_idx == 0)],
                        class_l[(jig_l == 0) & (d_idx == 0)],
                    )
                    class_irm_1 = compute_irm_penalty(
                        class_logit[(jig_l == 0) & (d_idx == 0)],
                        class_l[(jig_l == 0) & (d_idx == 0)],
                        criterion,
                    )
                    class_loss_2 = criterion(
                        class_logit[(jig_l == 0) & (d_idx == 1)],
                        class_l[(jig_l == 0) & (d_idx == 1)],
                    )
                    class_irm_2 = compute_irm_penalty(
                        class_logit[(jig_l == 0) & (d_idx == 1)],
                        class_l[(jig_l == 0) & (d_idx == 1)],
                        criterion,
                    )
                    class_loss_3 = criterion(
                        class_logit[(jig_l == 0) & (d_idx == 2)],
                        class_l[(jig_l == 0) & (d_idx == 2)],
                    )
                    class_irm_3 = compute_irm_penalty(
                        class_logit[(jig_l == 0) & (d_idx == 2)],
                        class_l[(jig_l == 0) & (d_idx == 2)],
                        criterion,
                    )
                    ...
                    rex_class = compute_rex_penalty(
                        class_loss_1, class_loss_2, class_loss_2
                    )
                ...
                class_loss_1 = criterion(
                    class_logit[(d_idx == 0)], class_l[(d_idx == 0)]
                )
                class_irm_1 = compute_irm_penalty(
                    class_logit[(d_idx == 0)], class_l[(d_idx == 0)], criterion
                )
                class_loss_2 = criterion(
                    class_logit[(d_idx == 1)], class_l[(d_idx == 1)]
                )
                class_irm_2 = compute_irm_penalty(
                    class_logit[(d_idx == 1)], class_l[(d_idx == 1)], criterion
                )
                class_loss_3 = criterion(
                    class_logit[(d_idx == 2)], class_l[(d_idx == 2)]
                )
                class_irm_3 = compute_irm_penalty(
                    class_logit[(d_idx == 2)], class_l[(d_idx == 2)], criterion
                )
                class_loss = class_loss_1 + class_loss_2 + class_loss_3
                irm_class = (class_irm_1 + class_irm_2 + class_irm_3) / 3
                rex_class = compute_rex_penalty(
            ...
            rex_loss = (
                self.rex_weight_class * rex_class
                + self.rex_weight_jigsaw * self.jig_weight * rex_jigsaw
            )
            irm_loss = (
                self.irm_weight_class * irm_class
                + self.irm_weight_jigsaw * self.jig_weight * irm_jigsaw
            )
            if (
                self.rex_weight_class == 0.0
                and self.rex_weight_jigsaw == 0.0
                and self.irm_weight_jigsaw == 0.0
                and self.irm_weight_class == 0.0
            ):
                loss = class_loss + jigsaw_loss * self.jig_weight
            elif self.irm_weight_jigsaw == 0.0 and self.irm_weight_class == 0.0:
                loss = class_loss + jigsaw_loss * self.jig_weight + rex_loss
            elif self.rex_weight_class == 0.0 and self.rex_weight_jigsaw == 0.0:
            ...
            self.logger.log(
                ...
                len(self.source_loader),
                {
                    ...
                    "rex loss class": rex_class.item(),
                    "rex loss jigsaw": rex_jigsaw.item(),
                    "rext total": rex_loss.item(),
                    "irm loss class": irm_class.item(),
                    "irm loss jigsaw": irm_jigsaw.item(),
                    "irm total": irm_loss.item(),
                },
                # ,"lambda": lambda_val},
                {
        ...
        with torch.no_grad():
            for phase, loader in self.test_loaders.items():
                ...
                if loader.dataset.isMulti():
                    ...
                    print(
                        "Single vs multi: %g %g"
                        % (float(single_acc) / total, float(class_correct) / total)
                    )
                else:
    ...
    def do_test_multi(self, loader):
        ...
        for it, ((data, jig_l, class_l), d_idx) in enumerate(loader):
            ...
            class_logits = torch.zeros(
                n_permutations, data.shape[0], self.n_classes
            ).to(self.device)
    ...
    def do_training(self):
        ...
        name = (
            self.args.prefix
            + "_"
            + str(self.args.source[0])
            + str(self.args.source[1])
            + str(self.args.source[2])
            + "_"
            + str(self.args.target)
            + "_eps%d_bs%d_lr%g_class%d_jigClass%d_rexWeightClass%g_rexWeightJig%g_irmWeightClass%g_irmWeightJig%g_jigWeight%g"
            % (
                self.args.epochs,
                self.args.batch_size,
                self.args.learning_rate,
                self.args.n_classes,
                self.args.jigsaw_n_classes,
                self.args.rex_weight_class,
                self.args.rex_weight_jigsaw,
                self.args.irm_weight_class,
                self.args.irm_weight_jigsaw,
                self.args.jig_weight,
            )
        )
        with open("./result_summary_txt/" + name + ".txt", "a+") as f:
            f.write(
                ...
                + str(val_res.max())
                + " test acc at best val acc: "
                + str(test_res[idx_best])
                + " max test: "
                + str(test_res.max())
            )
