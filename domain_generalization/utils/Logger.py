...
class Logger:
            ...
            if self.tf_logger:
                for k, v in losses.items():
                    self.tf_logger.scalar_summary(
                        "train/loss_%s" % k, v, self.current_iter
                    )
    ...
    def log_test(self, phase, accuracies):
        ...
        if self.tf_logger:
            for k, v in accuracies.items():
                self.tf_logger.scalar_summary(
                    "%s/acc_%s" % (phase, k), v, self.current_iter
                )
    ...
    def get_name_from_args(args):
        ...
        name = (
            "eps%d_bs%d_lr%g_class%d_jigClass%d_rexWeightClass%g_rexWeightJig%g_irmWeightClass%g_irmWeightJig%g_jigWeight%g"
            % (
                args.epochs,
                args.batch_size,
                args.learning_rate,
                ...
                args.rex_weight_class,
                args.rex_weight_jigsaw,
                args.rex_weight_class,
                args.rex_weight_jigsaw,
                args.jig_weight,
            )
